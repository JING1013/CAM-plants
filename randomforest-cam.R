...
head(rt)
#rt$stress<-factor(rt$stress,levels = c('Non-heat&drought stress',"Heat&drought stress","Heat stress"))
#rt$cam<-factor(rt$cam,levels = c('bbjt',"dnjt"))
#rt$Conduct<-as.numeric(as.character(rt$Conduct))
#rt$types<-factor(rt$types,levels=c("SSB-Non heat&drought stress","SSB-Heat&drought stress","SSB-Heat stress",
# "SAH-Non heat&drought stress","SAH-Heat&drought stress","SAH-Heat stress"))
str(rt)
...
bar<-data.frame(variable = c("whole",unique(rt$types)),
                Exp = c(65.83, 67.73, 58.15, 49.81,29.18,59.39,35.55))
...
circle<- data.frame(rt = colnames(rt)[4:12]) %>%
...
circle$rt<- factor(circle$rt, rev(colnames(rt)[4:12]))

r<-data.frame(rt=colnames(rt)[4:12],
              whole = (cor(rt[,-1:-2],method = c("spearman")))[1,2:10],
              SBB1 =  (cor(df1,method = c("spearman")))[1,2:10],
              SBB2 =  (cor(df2,method = c("spearman")))[1,2:10],
              SBB3 =  (cor(df3,method = c("spearman")))[1,2:10],
              SAH1 =  (cor(df4,method = c("spearman")))[1,2:10],
              SAH2 =  (cor(df5,method = c("spearman")))[1,2:10],
              SAH3 =  (cor(df6,method = c("spearman")))[1,2:10]) %>%
  melt(id = "rt", value.name = "Correlation");r
r$rt<-factor(r$rt,levels = rev(colnames(rt)[4:12]))
heatmap<- ggplot()+
